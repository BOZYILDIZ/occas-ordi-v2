...
const ALL_BRAND_VALUES = DEVICE_BRANDS.map((b) => b);
...
export default function EditComputerPage() {
  ...
  useEffect(() => {
    fetch(`/api/computers/${id}`)
      .then((r) => r.json())
      .then((c) => {
        // Déterminer si la marque est dans la liste ou "Autre"
        const brandInList = ALL_BRAND_VALUES.includes(c.brand ?? "");
        const brandValue  = c.brand ? (brandInList ? c.brand : "Autre") : "";
        const brandOther  = c.brand && !brandInList ? c.brand : "";

        // Déterminer si le modèle CPU est dans la liste ou "Autre"
        const cpuModels   = c.cpuBrand ? (CPU_MODELS[c.cpuBrand] ?? []) : [];
        const cpuInList   = cpuModels.some((m: { value: string }) => m.value === c.cpuModel);
        const cpuValue    = cpuInList ? c.cpuModel : (c.cpuBrand ? `Autre ${c.cpuBrand}` : "");
        const cpuOther    = !cpuInList ? c.cpuModel : "";
        ...
        setForm({
          ...
          cpuBrand:      c.cpuBrand     ?? "",
          cpuModel:      cpuValue,
          cpuModelOther: cpuOther,
          cpuGen:        c.cpuGen       ?? "",
          ramSize:       String(c.ramSize ?? ""),
          ramType:       c.ramType      ?? "",
          storageSize:   String(c.storageSize ?? ""),
          storageType:   c.storageType  ?? "",
          screenSize:    c.screenSize   ?? "",
          gpuModel:      c.gpuModel     ?? "",
          condition:     c.condition    ?? "",
          grade:         c.grade        ?? "",
          price:         c.price != null ? String(Number(c.price).toFixed(2)) : "",
          priceOld:      c.priceOld != null ? String(Number(c.priceOld).toFixed(2)) : "",
          os:            c.os           ?? "",
          color:         c.color        ?? "",
          notes:         c.notes        ?? "",
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(`/admin/computers/${id}`);
  };
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Modifier l&apos;appareil</h1>
          <p className="text-gray-500 text-sm font-mono mt-0.5">{form.sku}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="SKU / Référence">
              <input
                ...
                title="Le SKU ne peut pas être modifié"
              />
            </Field>
            <Field label="Type *">
              <Select
        ...
        <Section title="État & Prix">
          <div className="grid grid-cols-2 gap-4">
            ...
            {isNeuf ? (
              <div key="grade-neuf" className="flex items-center gap-2 px-3 py-2 bg-emerald-900/20 border border-emerald-800 rounded-lg">
                <span className="text-emerald-400 text-sm font-medium">Grade A+</span>
                <span className="text-emerald-600 text-xs">— automatique (neuf)</span>
              </div>
            ) : (
